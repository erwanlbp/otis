                    ...
                    for (const counter of counters) {
                        ...
                        await counter
                            ...
                            .then(async evs => {
                                ...
                                const monthlyEvents: { [monthTs: number]: CounterEventFirebaseDto[] } = _.groupBy(events.map(event => event.data()), (event: CounterEventFirebaseDto) => getMonthDate(event.timestamp));
                                for (const [month, monthEvents] of Object.entries(monthlyEvents)) {
                                    ...
                                    await admin.firestore().doc(`users/${user.id}/counters/${counter.id}/agg-monthly-events/${month}`).set(counterMonthlyEventsAggregation);
                                }
                            });
                    }
                });
