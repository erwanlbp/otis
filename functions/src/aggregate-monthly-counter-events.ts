  ...
  .onCreate((snapshot, context) => {
    ...
    return admin
      ...
      .then(monthlyAggregate => {
        ...
        const updatedAggregation: Partial<CounterMonthlyEventsAggregation> = {
          totalCount: existingAggregate.totalCount + getEventTypeValue(eventSaved.type, eventSaved.value),
        };
  ...
  .onUpdate((change, context) => {
    ...
    const updateBeforeMonthlyAgg = admin
      ...
      .then(monthlyAggregate => {
        const existingAggregate = monthlyAggregate.data() as CounterMonthlyEventsAggregation;
        const updatedAggregation: Partial<CounterMonthlyEventsAggregation> = {
          totalCount: existingAggregate.totalCount - getEventTypeValue(before.type, before.value),
        };
    ...
    const updateAfterMonthlyAgg = admin
      ...
      .then(monthlyAggregate => {
        ...
        const existingAggregate = monthlyAggregate.data() as CounterMonthlyEventsAggregation;
        const updatedAggregation: Partial<CounterMonthlyEventsAggregation> = {
          totalCount: existingAggregate.totalCount + getEventTypeValue(after.type, after.value),
        };
