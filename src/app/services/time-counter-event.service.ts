...
import {
  generateRandomTimeCounterEvent,
  TimeCounterEvent,
  toTimeCounterEvent,
  toTimeCounterEventDto,
} from '../interfaces/time-counter-event.interface';
...
export class TimeCounterEventService {
  constructor(private firestore: AngularFirestore, private authService: AuthService, private utilsService: UtilsService) {}
  ...
  fetchAllCounterEvents$(timeCounterName: string): Observable<TimeCounterEvent[]> {
    return this.userTimeCounterEventsDocument$(timeCounterName).pipe(
      ...
      map(events =>
        !events ? [] : events.map(event => toTimeCounterEvent(event.payload.doc.id, event.payload.doc.data(), timeCounterName)),
      ),
    );
  }

  fetchChunkTimeCounterEvents$(
    timeCounterName: string,
    chunkSize: number,
    lastEventStartTs: number = null,
  ): Observable<TimeCounterEvent[]> {
    // return of([
  ...
  assertValidStartDate(name: string, startDate: string, fromLastEvent: boolean = true): Promise<boolean> {
    ...
    return this.fetchChunkTimeCounterEvents$(name, fromLastEvent ? 1 : 2)
      .pipe(take(1))
      .toPromise()
      .then(chunk => {
