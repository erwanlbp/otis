...
export class TimeCounterMorePopoverComponent implements OnInit {
  ...
  events() {
    this.navController.navigateForward(`/time-counter-events/${this.timeCounter.name}`).then(() => this.close());
  }
  ...
  async startInPast() {
    ...
    alert.onDidDismiss().then(data => {
      ...
      return this.timeCounterEventService
        .assertValidStartDate(this.timeCounter.name, startDate)
        .then(valid => {
          if (!valid) {
            console.error('start date is not valid, nothing to do');
            return;
          }
          return this.timeCounterService.startCounter(
            this.timeCounter.name,
            moment(startDate, 'DD/MM/YYYY HH:mm:ss', true).toDate().getTime(),
          );
        })
        .catch(err => {
  ...
  async stopInPast() {
    ...
    alert.onDidDismiss().then(data => {
      ...
      return this.timeCounterService
        .stopCounter(this.timeCounter.name, moment(endDate, 'DD/MM/YYYY HH:mm:ss', true).toDate().getTime())
        .catch(err => {
