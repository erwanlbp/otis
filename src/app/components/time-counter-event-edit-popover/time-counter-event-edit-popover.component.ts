...
export class TimeCounterEventEditPopoverComponent implements OnInit {
  ...
  async modifyEvent() {
    ...
    await this.timeCounterEventService.updateEventTimestamps(
      this.event.timeCounterName,
      this.event.id,
      UtilsService.parseDateTimeToTimestamp(newStartDate),
      UtilsService.parseDateTimeToTimestamp(newEndDate),
    );
    await this.loaderService.dismissLoader();
    this.close();
  }
