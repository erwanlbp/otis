...
export class CounterEventsPage implements OnInit {
    ...
    loadMore() {
        this.eventService.fetchChunkCounterEvents$(this.counterName, CounterEventsPage.EVENTS_CHUNK_SIZE, this.counterEvents[this.counterEvents.length - 1].timestamp)
            .subscribe(events => {
                if (!events.length || events.length < CounterEventsPage.EVENTS_CHUNK_SIZE) {
                    this.noMoreData = true;
                }
