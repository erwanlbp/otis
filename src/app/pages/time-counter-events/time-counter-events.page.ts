...
export class TimeCounterEventsPage implements OnInit {
  ...
  constructor(private timeCounterEventService: TimeCounterEventService, private activatedRoute: ActivatedRoute) {}
  ...
  loadMore() {
    this.timeCounterEventService
      .fetchChunkTimeCounterEvents$(
        this.timeCounterName,
        TimeCounterEventsPage.EVENTS_CHUNK_SIZE,
        this.timeCounterEvents[this.timeCounterEvents.length - 1].startTimestamp,
      )
      .subscribe(events => {
        if (!events.length || events.length < TimeCounterEventsPage.EVENTS_CHUNK_SIZE) {
          this.noMoreData = true;
